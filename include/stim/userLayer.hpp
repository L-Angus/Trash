...
template <typename RFTYPE> struct RFWrapper {
public:
  void EnqueueTask(Task &&task) { static_cast<RFTYPE *>(this)->enqueue(std::move(task)); }
  void ExecuteTasks() { static_cast<RFTYPE *>(this)->execute(); }
  void ClearTasks() { static_cast<RFTYPE *>(this)->clear(); }
...
struct SDKWrapper {
  ...
  void EnqueueTask(Task &&task) { tasks[task.TaskID] = task.Action; }

  void ExecuteTasks() { SerilaizeExecutor (*this)(); }
...
public:
  ...
  void Load() {
    m_wrapper->EnqueueTask({1, [](STREAM_IN &inStream) { std::cout << "Task 1" << std::endl; }});
  }
  ...
  void Connect() {
    m_wrapper->EnqueueTask({4, [](STREAM_IN &inStream) { std::cout << "Task 4" << std::endl; }});
  }
  void Disconnect() {
    m_wrapper->EnqueueTask({5, [](STREAM_IN &inStream) { std::cout << "Task 5" << std::endl; }});
  }
  void Execute() {
...
class UserSDKImpl {
public:
  UserSDKImpl() : m_onessdk(std::make_unique<OnessdkImpl>()) { m_onessdk->SetType("CW"); };
  ~UserSDKImpl(){};
