...
namespace CSVUtils{
    inline namespace FileOperations{
        bool CheckFileExtension(const std::string& filename, const std::string& ext){
            ...
            if(file_ext.size() != ext.size()) return false;
            return std::equal(file_ext.begin(), file_ext.end(), ext.begin(), [](char a, char b){
        ...
        std::unique_ptr<BaseIO> CreateFileHandler(std::ifstream& in){
            if(!in.good()) throw std::runtime_error("Invalid file handle.");
...
#ifdef _WIN32
                IsNewLine = (row[pos] == '\r' && pos + 1 < str.size() && row[pos + 1] == '\n');
...
FileHandle::FileHandle(const std::string& filename) : m_file(filename), m_handle(m_file, std::ios::binary){
    if(!m_handle.is_open()){
...
FileManager::FileManager(const std::string& filename) : m_file_handle(std::make_unique<FileHandle>(filename)){}

void FileManager::OpenSourceFile(const std::string& filename){
    if(!m_file_handle) throw std::runtime_error("File handle is not initialized.");
    m_file_size = CSVUtils::FileOperations::CalFileByteSize(m_file_handle->GetHandle());
}

std::unique_ptr<BaseIO> FileManager::CreateFileHandler(){
    return CSVUtils::FileOperations::CreateFileHandler(m_file_handle->GetHandle());
}
...
void ParserImpl::ParseRows(const std::unique_ptr<BaseIO>& io, size_t filesize){
    ...
    if(m_read_buffer.size() == 0) throw std::runtime_error("No context.");
...
void ParserImpl::ParseColumns(const std::vector<std::string_view>& rows){
    ...
    for(size_t i = 0; i < rows.size(); ++i){
        ...
        if(!m_col_names.empty() && !ValidateColumnSize(columns)){
            auto err = std::string("Invalid column size: ") + std::to_string(columns.size()) + " at row " + std::to_string(i);
...
void ParserImpl::AsyncParseColumns(const std::vector<std::string_view>& rows, size_t workers){
    ...
    auto CheckAndSplitRow2Columns = [this, &rows](size_t j){
        ...
        if(!this->m_col_names.empty() && !ValidateColumnSize(columns)){
            auto err = std::string("Invalid column size: ") + std::to_string(columns.size()) + " at row " + std::to_string(j);
            throw std::runtime_error(err);
        }
    ...
    for(size_t i = 0; i < workers; ++i){
        worker_threads.emplace_back([&counter, &hasException, &exceptions, &CheckAndSplitRow2Columns, rows, this]{
            while(true){
                size_t j = counter.fetch_add(1);
                if(j >= rows.size() || hasException.load()) break;
                try{
...
void ParserImpl::OnOperationCallback(OperateStrategyCallback on_operation){
    if(on_operation) on_operation(m_data);
}

std::any ParserImpl::OnQueryCallback(QueryStrategyCallback on_query){
    if(on_query) return on_query(m_data);
    return std::any{};
}

bool ParserImpl::ValidateColumnSize(const std::vector<std::string_view>& columns){
    return columns.size() == m_col_names.size();
}

ParserStrategy::ParserStrategy() : m_parser_impl(std::make_unique<ParserImpl>()){}

void ParserStrategy::SetColumnNames(const std::vector<std::string_view>& columns){
...
void ParserStrategy::ClearAllCache(){
    m_parser_impl->ClearAllCache();
}
...
void SynchronousParser::ParseDataFromCSV(const std::unique_ptr<BaseIO>& io, size_t filesize){
    m_parser_impl->ParseRows(std::move(io), filesize);
    m_parser_impl->ParseColumns(m_parser_impl->GetAllRows());
}

AsynchronousParser::AsynchronousParser(size_t workers) : m_thread_workers(workers){}

void AsynchronousParser::ParseDataFromCSV(const std::unique_ptr<BaseIO>& io, size_t filesize){
    m_parser_impl->ParseRows(std::move(io), filesize);
    m_parser_impl->AsyncParseColumns(m_parser_impl->GetAllRows(), m_thread_workers);
}
...
std::vector<std::vector<std::string_view>> CSVParser::GetCSVData() const noexcept{
